...
The current `0.8.3` vertical slice is intentionally asset-light: it clones, recolors, and rescales base-game prefabs to prove the world-generation and Nautilus registration pipeline. It does **not** modify Subnautica's terrain mesh or biome lookup table, and the placeholder species do not yet have unique models, sounds, eggs, scan entries, or AI.

## Project status

**Prototype — use disposable saves.** Content definitions register at startup, but layout generation waits until a save has loaded. Version 0.8.3 retains the field-validated schema-3 migration, performance, boundary, and durable-regeneration model while retrying level-footprint landmark grounding only after a region's cell naturally streams. It never requests remote terrain batches. Existing schema-1 deterministic and schema-2 terrain-resolved manifests migrate without moving saved coordinates; exclusions introduced after a legacy layout was created remain compatibility warnings. Full manifest-wide terrain resolution remains unresolved because both tested remote batch-loading paths destabilized Subnautica's late-load phase.
...
The first representative seed-58 inspection on 2026-09-25 covered shallow (208 m), middle (343 m), and deep (459 m) regions. Flora attachment, fauna, collision avoidance, and escape paths passed, but all three central landmarks floated above terrain. Version 0.8.1's loaded-cell probe found terrain directly beneath AE1 and AE3 after streaming, proving a timing issue, while AE2's exact center remained over open water. Version 0.8.2 grounded AE1 and AE3, but incorrectly accepted 38.6- and 32.5-degree slopes using a single support point; AE2 still found no terrain within 72 metres. Version 0.8.3 replaces that check with a nine-point footprint requiring no more than 18 degrees of local slope or 0.75 metres of height variation, and expands the deterministic AE2 search to 144 metres with a higher probe origin. Its field test is pending.
...
- Delayed local landmark grounding with nine-point level-footprint validation and a deterministic 144-metre nearby-surface search, plus `ae_grounding` and bounded `ae_probe REGION` diagnostics; no remote cells are loaded.
...
For the 0.8.3 grounding test, visit each seed-58 region, wait at least 12 seconds after its landmark appears, and run `ae_grounding`. Each landmark should report `GROUNDED` with a nine-point support height spread no greater than 0.75 metres and maximum sampled slope no greater than 18 degrees. A landmark may move horizontally by up to 144 metres to the first deterministic acceptable loaded surface. Confirm visually that its full base is supported and level, it remains upright, and it is not buried, obstructed, or trapping the player. If a result fails, run `ae_probe REGION` before leaving the loaded cell; the read-only report compares terrain-mask and all-layer raycasts without loading remote cells.
...
Restart Subnautica before switching to a different save slot. Nautilus coordinated-spawn registrations are process-wide; version 0.8.3 safely refuses to mix a second manifest into the active session and stages regeneration only for the next process.
