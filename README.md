...
The first representative seed-58 inspection on 2026-09-25 covered shallow (208 m), middle (343 m), and deep (459 m) regions. Flora attachment, fauna, collision avoidance, and escape paths passed, but all three central landmarks floated above terrain. Version 0.8.4's level foundations passed visual and 9/9 terrain-hit inspection for AE1 and AE3. AE2 remained over void through 0.8.6 because coordinated flora objects could instantiate about 100 metres away while their terrain colliders remained unloaded at the void-centered player position. Version 0.8.7 registers the Glass Arch and its field-check teleport at a deterministic ring point beside the nearest Prism Kelp, causing the correct terrain cell to stream before the bounded foundation scan. The manifest coordinate remains unchanged. Its field test is pending.
...
For the 0.8.7 grounding test, `goto ae2` uses the Glass Arch's deterministic runtime streaming anchor beside the nearest Prism Kelp rather than the void-centered manifest coordinate. Stay there for at least 12 seconds and run `ae_grounding`. It should report `GROUNDED` with final coordinates, a level-foundation size, terrain-hit count, terrain relief, and maximum sampled terrain slope. Confirm visually that the arch is upright and fully seated, the foundation intersects seabed rather than floating, and neither object is buried, obstructed, intersecting flora, or trapping the player. Recheck AE1 and AE3 only for regression. If a result fails, run `ae_probe 2` before leaving the loaded cell.
...
Restart Subnautica before switching to a different save slot. Nautilus coordinated-spawn registrations are process-wide; version 0.8.7 safely refuses to mix a second manifest into the active session and stages regeneration only for the next process.
