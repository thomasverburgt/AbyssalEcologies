...
- Local streamed-cell landmark grounding diagnostics with `ae_grounding` and bounded `ae_probe REGION` collider reports; no remote cells are loaded.
...
For the 0.8.1 probe test, visit each seed-58 region and run `ae_probe 1`, `ae_probe 2`, and `ae_probe 3` after its cell and landmark appear. Each report compares a local 320 m probe and a full-depth 1,800 m probe using both the game's terrain mask and all physics layers, then lists at most 12 hits. This is read-only instrumentation and never loads a remote cell.
...
Restart Subnautica before switching to a different save slot. Nautilus coordinated-spawn registrations are process-wide; version 0.8.1 safely refuses to mix a second manifest into the active session and stages regeneration only for the next process.
