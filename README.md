...
The first representative seed-58 inspection on 2026-09-25 covered shallow (208 m), middle (343 m), and deep (459 m) regions. Flora attachment, fauna, collision avoidance, and escape paths passed, but all three central landmarks floated above terrain. Version 0.8.0 attempted bounded local raycasts at landmark instantiation, but all three returned zero terrain-layer hits and remained floating on 2026-09-26. Version 0.8.1's loaded-cell probe then found terrain directly beneath AE1 and AE3 after streaming, proving a timing issue, while AE2's exact center remained over open water. Version 0.8.2 therefore retries grounding for about ten seconds and deterministically searches loaded terrain within 72 metres when the exact center has no acceptable surface. Its field test is pending.
...
- Delayed local landmark grounding with a deterministic 72-metre nearby-surface search, plus `ae_grounding` and bounded `ae_probe REGION` diagnostics; no remote cells are loaded.
...
For the 0.8.2 grounding test, visit each seed-58 region, wait at least 12 seconds after its landmark appears, and run `ae_grounding`. Each landmark should report `GROUNDED`; AE1 and AE3 should normally settle vertically, while AE2 may move horizontally by up to 72 metres to the first deterministic acceptable loaded surface. Confirm visually that each landmark is supported, upright, not buried, unobstructed, and does not trap the player. If a result fails, run `ae_probe REGION` before leaving the loaded cell; the read-only report compares terrain-mask and all-layer raycasts without loading remote cells.
...
Restart Subnautica before switching to a different save slot. Nautilus coordinated-spawn registrations are process-wide; version 0.8.2 safely refuses to mix a second manifest into the active session and stages regeneration only for the next process.
