...
For the 0.8.9 lifecycle test, run `goto ae2`, wait 12 seconds, and require `ae_grounding` to report the Glass Arch `GROUNDED` with 9/9 support hits. Run `ae_perf`, travel far enough away for the Glass Arch to unload, then return with `goto ae2`. The seamount must remain throughout, no duplicate landform may appear, the Prism Kelp and arch must remain seated, and `ae_perf` must still pass. Fully quit and reload once to verify the session-resident landform is recreated exactly once.
