...
Begin the Cinder Ray original-fauna slice without polishing or redesigning the accepted Glassfin prototype. Replace the Rabbit Ray-derived visible model with project-owned procedural geometry, an articulated rig, texture/icon, animation, sound, scanner/databank entry, and distinct egg while retaining the proven gameplay shell and an explicit rollback switch. Add a bounded heat-adapted ecological display, then repeat streaming, performance, scan, egg, save/restart, and containment checks in the Ember Trench.
