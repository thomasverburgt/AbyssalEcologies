...
| Implemented; field test pending | Local landmark grounding | Anchor broad central landmarks on level support only after their cells naturally stream | 0.8.3 proved no naturally level footprint was available in the loaded candidate set; 0.8.4 creates a level footprint-sized foundation intersecting at least five of nine terrain samples and searches loaded sites within 144 metres |
...
Field-test 0.8.4 at all three naturally streamed seed-58 regions. After each landmark appears, wait at least 12 seconds and run `ae_grounding`; require `GROUNDED`, at least five of nine terrain samples, and visual confirmation that the landmark is upright and fully seated on a level foundation, the foundation intersects seabed rather than floating, and neither object is buried, obstructed, or trapping the player. A landmark may move horizontally by up to 144 metres. If any result fails, capture `ae_probe REGION` before leaving the loaded cell. Do not force remote batch loading.
