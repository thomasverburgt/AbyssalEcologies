...
| Partial; AE2 retest pending | Local landmark grounding | Anchor broad central landmarks on level support only after their cells naturally stream | 0.8.4 level foundations passed AE1/AE3 with 9/9 terrain hits; 0.8.7 moves the Glass Arch's runtime spawn and field teleport from the void-centered manifest point to a deterministic nearby-flora streaming anchor before probing |
...
Field-test 0.8.7 at seed-58 AE2 using the relocated `goto ae2` streaming anchor. Remain there for at least 12 seconds, run `ae_grounding`, and require `GROUNDED`, final coordinates, at least five of nine terrain samples, and visual confirmation that the Glass Arch is fully seated on its level foundation without flora intersection, floating foundation, burial, obstruction, or trapping. Recheck AE1/AE3 only for regression. If AE2 fails, capture `ae_probe 2` before leaving the loaded cell. Do not force remote batch loading.
