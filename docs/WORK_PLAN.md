...
| Implemented; field test pending | Local landmark grounding | Ground central landmarks only after their cells naturally stream | 0.8.1 found loaded terrain directly beneath AE1/AE3 but open water at AE2's exact center; 0.8.2 retries for about ten seconds and searches deterministic samples within 72 metres without loading remote cells |
...
Field-test 0.8.2 at all three naturally streamed seed-58 regions. After each landmark appears, wait at least 12 seconds and run `ae_grounding`; require `GROUNDED` plus visual confirmation that the object is supported, upright, not buried, unobstructed, and escapable. AE2 may move horizontally by up to 72 metres. If any result fails, capture `ae_probe REGION` before leaving the loaded cell. Do not force remote batch loading.
