...
| Partial; AE2 retest pending | Local landmark grounding | Anchor broad central landmarks on level support only after their cells naturally stream | 0.8.4 level foundations passed AE1/AE3 with 9/9 terrain hits; 0.8.5 adds deterministic sites outward from each region's terrain-backed flora to move a void-centered landmark before the radial fallback |
...
Field-test 0.8.5 at seed-58 AE2. Remain in the region for at least 12 seconds, run `ae_grounding`, and require `GROUNDED`, final coordinates, at least five of nine terrain samples, and visual confirmation that the relocated Glass Arch is fully seated on its level foundation without flora intersection, floating foundation, burial, obstruction, or trapping. Recheck AE1/AE3 only for regression. If AE2 fails, capture `ae_probe 2` before leaving the loaded cell. Do not force remote batch loading.
