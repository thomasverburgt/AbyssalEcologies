...
| Implemented; field test pending | Local landmark grounding | Ground broad central landmarks on level support only after their cells naturally stream | 0.8.2 grounded AE1/AE3 on visibly unsuitable 38.6/32.5-degree slopes and failed AE2; 0.8.3 requires a nine-point footprint at no more than 18 degrees and 0.75 metres of height variation while searching loaded terrain within 144 metres |
...
Field-test 0.8.3 at all three naturally streamed seed-58 regions. After each landmark appears, wait at least 12 seconds and run `ae_grounding`; require `GROUNDED`, a reported nine-point support spread no greater than 0.75 metres, maximum slope no greater than 18 degrees, and visual confirmation that the full base is supported and level, upright, not buried, unobstructed, and escapable. A landmark may move horizontally by up to 144 metres. If any result fails, capture `ae_probe REGION` before leaving the loaded cell. Do not force remote batch loading.
