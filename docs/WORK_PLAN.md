...
| Partial; AE2 seamount test pending | Local landmark and void-region grounding | Anchor broad landmarks and provide bounded surface support for an intentionally retained void layout | 0.8.4 level foundations passed AE1/AE3 with 9/9 hits; after relocation candidates proved terrainless, 0.8.8 adds a streamed 132-metre-radius seamount under AE2 and surface-grounds its 28 flora placements |
...
Field-test 0.8.8 at seed-58 AE2. Remain there for at least 12 seconds and require `ae_grounding` to report the Glass Arch `GROUNDED` with 9/9 hits. Inspect the entire 230-metre region: the oblate seamount must be visible, collidable, and unload with AE2; all 28 Prism Kelp must touch its curved surface; fauna must remain in the water above; and the arch must be seated without burial, obstruction, or trapping. Run `ae_perf`, then revisit AE1/AE3 for regression. Do not force remote batch loading or mutate baked terrain.
