...
| Failed; diagnosing | Local landmark grounding | Ground central landmarks only after their cells naturally stream | Seed-58 baseline and 0.8.0 both left all three landmarks floating; every 0.8.0 terrain-mask raycast returned zero hits, so 0.8.1 compares local/full-depth and terrain-mask/all-layer hits before another fix |
...
Field-test `ae_probe` in 0.8.1 at all three naturally streamed seed-58 regions. Capture terrain-mask and all-layer hit counts plus bounded collider details, identify the actual seabed collision layer and whether it is available after streaming, then implement one evidence-based local retry or layer correction. Do not force remote batch loading.
