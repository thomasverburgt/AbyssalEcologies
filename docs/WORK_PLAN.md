...
Versions 0.9.0 through 0.11.0 establish project-owned procedural models, articulated rigs, generated icons/textures, synthesized calls, scanner entries, and distinct eggs for Glassfin, Cinder Ray, and Lantern Skate. Proven vanilla shells remain temporarily responsible for locomotion and save integration, and each species has an explicit visual rollback switch. Functional field validation passed for all three on 2026-09-26; containment hatching, distinct bounded behavior for Cinder Ray and Lantern Skate, and final art direction remain future work.
...
Build a diagnostic 2 by 2 expedition terrain grid from chunks `(0,0)`, `(1,0)`, `(0,1)`, and `(1,1)`. Each chunk must retain its own mesh, collider, descriptor, and bounded metrics while sharing byte-identical border heights with its neighbors. Add removable temporary inspection lights and seam markers at the deep staging pocket, report maximum runtime border gap and total geometry/memory, and remove the entire grid atomically. Validate walking/swimming and collision across both east-west and north-south seams, cleanup, save/full-restart inactivity, and unchanged home-world manifest/performance before implementing automatic streaming.
