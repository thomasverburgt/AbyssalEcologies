...
| Partial; AE2 lifecycle retest pending | Local landmark and void-region grounding | Anchor broad landmarks and provide bounded surface support for an intentionally retained void layout | 0.8.8 passed initial AE2 grounding/performance but destroyed the landform before outer cells unloaded; 0.8.9 keeps one duplicate-guarded seamount for the loaded session |
...
Field-test 0.8.9 at seed-58 AE2. Require initial 9/9 Glass Arch grounding and passing `ae_perf`; move far enough away for the central landmark to unload while outer objects stream independently; confirm the seamount never disappears; return with `goto ae2` and confirm no duplicate, floating flora, burial, obstruction, or trapping. Run `ae_perf` again, then fully restart and verify exactly one recreated landform. Recheck AE1/AE3 only for regression. Do not force remote batch loading or mutate baked terrain.
